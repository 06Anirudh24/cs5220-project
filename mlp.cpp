// mlp.cpp — Single-node 3-layer MLP for MNIST with momentum SGD
// Architecture: 784 -> 1024 -> 1024 -> 1024 -> 10
// Compile: g++ -O2 -std=c++17 mlp.cpp -o mlp
// Run:     ./mlp
...
float grad_norm(const std::vector<std::vector<float>*>& grads) {
    double sum = 0.0;
    for (auto* g : grads)
        for (float v : *g) sum += (double)v * v;
...
struct MLP {
    ...
    float forward(const float* x, const uint8_t* labels, int bs) {
        ...
        float loss = 0.0f;
        for (int i = 0; i < bs; i++) {
            float p = a4[i*OUTPUT_DIM + labels[i]];
...
int main() {
    ...
    // Print CSV header — easy to parse later
    std::cout << "epoch,loss,test_acc,fwd_s,bwd_s,sgd_s,epoch_s,grad_norm\n";
    ...
    for (int epoch = 0; epoch < EPOCHS; epoch++) {
        ...
        double t_fwd = 0, t_bwd = 0, t_sgd = 0;
        ...
        // Gradient norm across all parameter gradients
        float gnorm = grad_norm({
            &model.dW1, &model.db1,
            &model.dW2, &model.db2,
            &model.dW3, &model.db3,
            &model.dW4, &model.db4
        ...
        // Flush so output appears live even when redirected
        std::cout.flush();
    }
